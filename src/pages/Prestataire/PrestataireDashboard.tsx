...
import { Plus, TrendingUp, Clock, CheckCircle, Star, DollarSign, Eye, MessageSquare, LayoutDashboard, Inbox, Briefcase, User } from 'lucide-react'
...
const demandes = [
  { id: 1, titre: 'Reparation fuite salle de bain',  statut: 'en_cours' as const,   montant: 15000, dateIntervention: '2026-05-28', client: 'Marie K.',   localisation: 'Yaounde Centre' },
  ...
  { id: 3, titre: 'Remplacement robinets cuisine',    statut: 'terminee' as const,   montant: 10000, dateIntervention: '2026-05-20', client: 'Joelle A.',  localisation: 'Mvan'           },
]

const mesPrestations = [
  { id: 1, titre: 'Reparation fuite urgente',        avisCount: 34, vues: 120, prix: 15000 },
  { id: 2, titre: 'Installation sanitaire complete', avisCount: 22, vues: 87,  prix: 60000 },
]
...
export function PrestataireDashboard() {
  ...
  const cardStats = [
    { label: t.dashboard.active,  value: '3',                 icon: Clock,       color: 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400',     delta: '+2 cette semaine' },
    ...
    { label: t.dashboard.balance, value: formatPrice(125000), icon: DollarSign,  color: 'bg-orange-50 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400', delta: 'Retrait possible' },
    { label: 'Note moyenne',      value: '4.8 / 5',           icon: Star,        color: 'bg-amber-50 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400',   delta: '34 avis'          },
  ]

  const items: SidebarItem[] = [
    { icon: LayoutDashboard, label: 'Tableau de bord', href: '/prestataire' },
    { icon: Inbox,    label: 'Demandes recues',  onClick: () => setTab('demandes'),    active: tab === 'demandes'    },
    { icon: Briefcase, label: 'Mes prestations', onClick: () => setTab('prestations'), active: tab === 'prestations' },
    { separator: true },
    { icon: Plus, label: t.dashboard.new_request, href: '/prestataire/nouvelle-prestation' },
    { separator: true },
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-slate-500 dark:text-slate-400 text-sm mt-0.5">Gerez vos prestations et repondez aux demandes</p>
          </div>
            ...
            {demandes.map(d => (
              <div key={d.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 flex flex-col sm:flex-row sm:items-center gap-4 hover:border-blue-200 dark:hover:border-blue-700 hover:shadow-sm transition-all">
                ...
                <div className="flex items-center gap-3 shrink-0">
                  ...
                  {d.statut === 'en_attente' && <Button variant="cta" size="sm">Accepter</Button>}
        ...
        {tab === 'prestations' && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {mesPrestations.map(p => (
              <div key={p.id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-100 dark:border-slate-800 p-5 hover:border-blue-200 dark:hover:border-blue-700 hover:shadow-sm transition-all">
                <div className="flex items-start justify-between gap-3 mb-3">
                  <h3 className="font-display font-semibold text-slate-900 dark:text-white">{p.titre}</h3>
                  <p className="font-display font-bold text-blue-700 dark:text-blue-400 shrink-0">{formatPrice(p.prix)}</p>
                </div>
            ...
            <Link
              ...
              className="bg-blue-50 dark:bg-blue-900/20 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded-2xl border-2 border-dashed border-blue-200 dark:border-blue-700 p-5 flex flex-col items-center justify-center gap-2 min-h-[160px] cursor-pointer transition-colors"
            >
              <div className="w-10 h-10 bg-blue-100 dark:bg-blue-900/50 rounded-xl flex items-center justify-center">
                <Plus className="w-5 h-5 text-blue-600 dark:text-blue-400" aria-hidden />
              </div>
              <p className="font-semibold text-blue-600 dark:text-blue-400 text-sm">Ajouter une prestation</p>
            </Link>
          </div>
        )}
